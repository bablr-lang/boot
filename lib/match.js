...
import { buildOpenNodeTag, coverFlags, nodeFlags } from '@bablr/agast-helpers/builders';
...
export class Match {
  ...
  get isNode() {
    ...
    return covers.get(sym.node).has(name) && !covers.has(name);
  }
  ...
  static from(language, id, attrs = {}) {
    ...
    const isNode = covers.get(sym.node).has(name);
    ...
    path.node = buildNode(buildOpenNodeTag(isCover ? coverFlags : nodeFlags, name));
  ...
  generate(id, attrs) {
    ...
    const { name } = id;
    const isCover = covers.has(name);
    const isNode = covers.get(sym.node).has(name) && !isCover;
