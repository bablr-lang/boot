import { createNode } from '@bablr/agast-helpers/path';
...
export class Match {
  ...
  static from(language, id, attrs = {}) {
    ...
    path.node = createNode(buildOpenNodeTag(isCover ? coverFlags : nodeFlags, type));
  ...
  generate(id, attrs) {
    ...
    if (isNode) {
      ...
      path.node = createNode(buildOpenNodeTag(nodeFlags, type));
    }
