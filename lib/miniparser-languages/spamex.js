...
const grammar = class SpamexMiniparserGrammar {
  ...
  TokenMatcher(p) {
    ...
    const tagName = p.eatProduction('Identifier', { path: 'tagName' });
    ...
    const attrs = sp ? p.eatProduction('Attributes', { path: '[attrs]' }) : [];

    sp = attrs.length ? p.eatMatch(_) : sp;

    const props = sp && p.match('{') ? p.eatProduction('Props', { path: 'props' }) : [];
  ...
  KeyValueAttribute(p) {
    ...
    p.eat('=');
    p.eatMatch(_);
    const str = p.eatProduction('String', { path: 'value' });
  ...
  Argument(p) {
    ...
    p.eat(':');
    p.eatMatch(_);
    const value = p.eatProduction('Expression', { path: 'value' });
  ...
  RegexMatcher(p) {
    ...
    const alternatives = p.eatProduction('Regex:Alternatives', { path: 'alternatives' });
    p.eat('/', { endSpan: 'Expression' });
    const flags = p.eatProduction('Regex:Flags', { path: 'flags' });
