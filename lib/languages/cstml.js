...
export const grammar = class CSTMLMiniparserGrammar {
  ...
  ReferenceFlags(p) {
    let array = !!p.eatMatch('[]', PN, { path: 'arrayToken:' });
    let expression = !!p.eatMatch('+', PN, { path: 'expressionToken:' });
    let intrinsic = !!p.eatMatch('*', PN, { path: 'intrinsicToken:' });
    let hasGap = !!p.eatMatch('$', PN, { path: 'hasGapToken:' });
    if (intrinsic && hasGap) p.fail();

    return { attrs: { array, expression, intrinsic, hasGap } };
  }
...
export default { name, canonicalURL, dependencies, covers, grammar, cookEscape: null };
