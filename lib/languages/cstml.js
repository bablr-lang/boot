...
export const grammar = class CSTMLMiniparserGrammar {
  ...
  NodeChild(p, _, props) {
    ...
    if (token) {
      ...
      if (p.match(/<\*?#/y)) {
        p.eatProduction('Node');
      } else if (p.match(/[a-zA-Z`\\_\u{80}-\u{10ffff}.]|[.#@]/uy)) {
  ...
  OpenNodeTag(p) {
    ...
    let sp = null;
    let isFragment = !p.match(/['"a-zA-Z`\\_\u{80}-\u{10ffff}.]/uy);
  ...
  TagType(p) {
    if (
      p.match(/['"]|[a-zA-Z`\\_\u{80}-\u{10ffff}.]+:/uy) ||
  ...
  IdentifierContent(p, _, props) {
    ...
    do {
      if ((esc = p.match('\\'))) {
        ...
        if (!quoted) {
          lit = p.eatMatchLiteral(/[a-zA-Z_\u{80}-\u{10ffff}][a-zA-Z0-9_\u{80}-\u{10ffff}-]*/uy);
        } else {
