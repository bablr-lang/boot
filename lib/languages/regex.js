...
const grammar = class RegexMiniparserGrammar {
  ...
  Character(p) {
    ...
    const esc = p.eatMatch('\\', 'Escape', { path: 'escape' });
  ...
  CharacterClass(p) {
    ...
    p.eat(']', PN, { path: 'close', endSpan: 'CharacterClass' });
  }
  ...
  // @Node
  Quantifier(p) {
    // Likely there's a path problem here
    if (!p.element) {
