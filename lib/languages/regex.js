...
const cookEscape = (path, escape) => {
  ...
  let specialMatch = getSpecialPattern(path).exec(escape.slice(1));
...
const grammar = class RegexMiniparserGrammar {
  ...
  Element(p) {
    ...
    if (p.match(/[*+?]|{\d+,?\d*}/y)) {
      p.shiftProduction('Quantifier', innerAttrs);
    }
  }
  ...
  // @Node
  Character(p, path) {
    const specialPattern = getSpecialPattern(path);
  ...
  CharacterClass(p) {
    ...
    let noNegate = !!p.eatMatch('^', KW, { path: 'negate', boolean: true });

    while (p.match(/./sy)) {
      p.eatProduction('CharacterClassElement', { path: '[elements]', noNegate });
      noNegate = false;
    }
  ...
  // @Node
  CharacterClassRange(p, { attrs: { noNegate } }) {
    p.eatProduction('Character', { path: 'min', noNegate });
