import { when } from '@bablr/agast-helpers/object';
...
export const grammar = class RegexMiniparserGrammar {
  ...
  WordBoundaryAssertion(p) {
    let attrs;
    if (p.eatMatch('\\', PN, { path: 'escapeToken:' })) {
  ...
  Character(p) {
    ...
    if (p.match('\\')) {
      if (p.match(new RegExp(String.raw`\\[unrt0]|${specialPattern.source}`, 'y'))) {
  ...
  EscapeSequence(p) {
    ...
    } else if ((m_ = p.eatMatch(specialPattern, KW, { path: 'code:' }))) {
      cooked = m_;
    } else if (p.match(/[ux]/y)) {
  ...
  EscapeCode(p) {
    if (p.eatMatch('u', KW, { path: 'type:' })) {
      if (p.eatMatch('{', PN, { path: 'openToken:' })) {
        p.eatMatch(/[\da-fA-F]+/y, 'UnsignedHexInteger', { path: 'value:' });
        p.eat('}', PN, { path: 'closeToken:' });
      } else {
        p.eatMatch(/[\da-fA-F]{4}/y, 'UnsignedHexInteger', { path: 'value:' });
      }
    } else if (p.eatMatch('x', KW, { path: 'type:' })) {
      p.eat(/[\da-fA-F]{2}/y, 'UnsignedHexInteger', { path: 'value:' });
    }
  }
