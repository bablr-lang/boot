...
const covers = buildCovers({
  [sym.node]: ['Attribute', 'Argument', 'Identifier', 'Props', 'Expression'],
  Attribute: ['StringAttribute', 'BooleanAttribute'],
  Matcher: ['NodeMatcher', 'TokenMatcher', 'TriviaTokenMatcher', 'StringMatcher'],
  StringMatcher: ['String', 'Regex'],
  Expression: ['Matcher', 'Boolean'],
...
const grammar = class SpamexMiniparserGrammar {
  ...
  MatchablesArrayProps(p) {
    ...
    while (sp && p.match(/./sy)) {
      p.eatProduction('Matcher', { path: '[values]' });
      sp = p.eatMatchTrivia(_);
    }
  ...
  ObjectProps(p) {
    ...
    let first = true;
    while ((first || p.match(/\s*,/y)) && p.match(/./sy)) {
      if (!first) {
        p.eatMatchTrivia(_);
