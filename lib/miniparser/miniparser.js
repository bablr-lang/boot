...
class TemplateParser {
  ...
  eatProduction(tagType, attrs = {}) {
    ...
    const lang = this.resolveDependent(langName);

    return this.eval(lang, type, attrs);
  }

  eatProductions(tagType, attrs = {}) {
    let type;
    let langName = this.language.name;
    const parts = tagType.split(':');
    if (parts.length === 1) {
  ...
  eval(language, type, attrs = {}) {
    ...
    if (this.atExpression && covers.get(node).has(type)) {
      ...
      if (parentPath.children) {
        parentPath.children.push({ type: 'GapNodeTag' });
      }
      ...
      returnValue = isNode ? { tagName: type, children, properties: result, attrs } : result;
    }
