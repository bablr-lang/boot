...
const add = (obj, path, value) => {
  ...
  if (pathIsArray) {
    ...
    obj[name] = btree.add(obj[name], value);
  } else {
    if (hasOwn(obj, name)) {
  ...
  for (const child of miniNode.children) {
    if (child.type === OpenNodeTag || child.type === CloseNodeTag) {
      ...
      if (isArray && !hasOwn(properties, name)) {
        ...
        children = btree.add(children, newRef);
        children = btree.add(children, arrayTag);

        add(properties, { name }, []);
      }

      add(properties, path, agASTNode);
      children = btree.add(children, { type: ReferenceTag, value: { name, isArray } });
    } else if (child.type === Trivia) {
      children = btree.add(children, {
        type: EmbeddedNode,
        value: getAgASTValue(
          resolvedLanguage,
      ...
      children = btree.add(children, child);
    }
  }

  children = btree.add(children, buildNodeCloseTag());
