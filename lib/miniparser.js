...
class TemplateParser {
  ...
  eval(gap) {
    ...
    const { type } = gap;
    const { language, production } = type;
    ...
    const isNode = covers.get(sym.node).has(production);
    const { path } = this;
    ...
    if (this.atExpression && covers.get(sym.node).has(production)) {
      ...
      if ((parentPath.node.children && isNode) || covers.has(production)) {
        parentPath.node.children.push({ type: 'Gap', value: gap.attrs.path });
        set(parentPath.node.properties, gap.attrs.path, null);
      }
    } else {
      ...
      const node = { language, production, children, properties, attrs, gap: undefined };
