...
class TemplateParser {
  ...
  eval(gap) {
    ...
    if (this.atExpression && covers.get(sym.node).has(type)) {
      ...
      if (parentPath?.children && isNode) {
        parentPath.children.push({ type: 'Reference', value: this.path.attrs.path.trim() });
      }
    ...
    if (isNode && parentPath) {
      ...
      if (attrs.path?.trim().startsWith('[')) {
        const path = stripPathBraces(attrs.path);
        if (!properties[path]) {
          properties[path] = [];
        }
        properties[path][resolver.resolve(path)] = returnValue;
      } else if (attrs.path) {
        properties[attrs.path] = returnValue;
      } else {
        throw new Error('node needs a path');
      }
    }
  ...
  eat(pattern, type, attrs = {}) {
    ...
    this.path.children.push({
      tagName: type,
      children: [{ type: 'String', value: result }],
      properties: {},
      attrs,
      gap: null, // do I need a gap?
    });

    return result;
  }

  match(pattern, type, attrs = {}) {
