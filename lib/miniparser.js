...
class TemplateParser {
  ...
  eval(gap) {
    ...
    if (this.atExpression && covers.get(sym.node).has(type)) {
      ...
      if (parentPath.children) {
        ...
        set(parentPath.properties, path, null);
      }
      ...
      const node = { tagName, children, properties, attrs, gap: undefined };

      if (parentPath?.children && isNode) {
        parentPath.children.push({ type: 'Reference', value: path });
        set(parentPath.properties, path, node);
      }
  ...
  eat(pattern, type, attrs) {
    ...
    if (!isObject(attrs) || !attrs.path?.trim()) throw new Error('a node must have a path');
    ...
    set(this.path.properties, attrs.path, {
      tagName: { language: this.language.name, type },
      children: [result],
      attrs: {},
      gap: undefined,
    });
