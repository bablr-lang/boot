...
const { set, isRegex, parsePath, getPrototypeOf, buildNode, buildId } = require('./utils.js');
...
class TemplateParser {
  ...
  get matchIsNode() {
    return this.language.covers.get(sym.node).has(this.m.type);
  }

  get matchIsCover() {
    return this.language.covers.has(this.node.type);
  }
  ...
  eval(id, attrs = {}) {
    ...
    if (this.atExpression && isNode) {
      ...
      if ((parentPath.node.children && isNode) || covers.has(type)) {
        parentPath.node.children.push({ type: 'Gap', value: attrs.path });
        set(parentPath.node.properties, attrs.path, null);
      }
    } else {
      const result = getPrototypeOf(grammar)[type].call(grammar, this, path);

      const node = buildNode(id, children, properties, result?.attrs);

      if (parentPath?.node.children && isNode && !covers.has(type)) {
        parentPath.node.children.push(ref(parentProperty));
  ...
  eatProduction(id, attrs = {}) {
    return this.eval(this.buildId(id), attrs);
  }
  ...
  eatEscape(pattern) {
    ...
    this.node.children.push(esc(result, this.grammar.cookEscape(this.path, result)));
