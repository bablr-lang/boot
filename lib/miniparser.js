...
const { stripPathBraces, resolveDependentLanguage } = require('./utils.js');
...
class TemplateParser {
  ...
  eval(gap) {
    ...
    if (this.atExpression && covers.get(sym.node).has(type)) {
      ...
      const { children, properties, parentProperty } = this.path;
      const attrs = result?.attrs || {};

      if (parentPath?.children && isNode) {
        parentPath.children.push({ type: 'ReferenceTag', value: { path: this.path.attrs.path } });
      }
    ...
    if (isNode && parentPath) {
      const { resolver, properties } = parentPath;

      let result;
      if ((result = stripPathBraces(attrs.path))) {
        if (!properties[result]) {
          properties[result] = [];
        }
        properties[result][resolver.resolve(result)] = returnValue;
      } else {
