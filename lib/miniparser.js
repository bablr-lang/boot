...
export class TemplateParser {
      ...
      if (isNode || !parentPath) {
        ...
        if (result?.attrs) {
          ...
          node.tags = Tags.replaceAt(
            0,
            node.tags,
            buildOpenNodeTag(isCover ? coverFlags : nodeFlags, node.type, null, result.attrs),
          );
        }
  ...
  shiftProduction(id, attrs = {}, props = {}) {
    ...
    let ref = Tags.getAt(-1, node.tags).value.tags[0];
