...
const esc = (value) => ({ type: 'Escape', value });
...
class TemplateParser {
  ...
  eval(id, attrs = {}) {
    ...
    if (this.atExpression && covers.get(sym.node).has(type)) {
      ...
      const result = getPrototypeOf(grammar)[type].call(grammar, this, attrs);
  ...
  eatProduction(id, attrs = {}) {
    return this.eval(buildId(id), attrs);
  }
  ...
  eat(pattern, type, attrs) {
    ...
    const id = { language: this.language.name, type };

    set(this.node.properties, attrs.path, buildNode(id, [str(result)]));
  ...
  eatEscape(pattern) {
    ...
    this.node.children.push(esc(result));
